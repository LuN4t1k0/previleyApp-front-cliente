...
// import PagexDetalleConfig from "@/config/module/mora/moraPresuntaDetalle.config";
import PagexDetalleConfig from "@/config/module/pagex/PagexDetalle.config";
...
const PagexDetallePageContent = () => {
  ...
  // Realtime: Detalle Pagex CRUD → refrescar listado
  const { data: session } = useSession();
  const { socket } = useSocket(session?.accessToken);
  useRealtimeEntity(socket, "detallePagex", {
    onCreated: () => fetchData(),
    onUpdated: () => fetchData?.(),
    onDeleted: () => fetchData(),
  });
...
const {
  ...
  fetchFn: ({ offset, limit }) =>
    fetchPage({
      offset,
      limit,
      extraParams: convertFiltersToQueryParams(filters, PagexDetalleConfig.filters),
    }),
  limit: 20,
  ...
  return (
    <div className="w-full px-2 sm:px-6 lg:px-8 pt-4 relative">
      ...
      <GenericTableWithDetail
        ...
        actionHandlers={actionHandlers}
        role={role}
        columnOrder={PagexDetalleConfig.columnOrder}
        useInfiniteScroll={PagexDetalleConfig.useInfiniteScroll}
        // NUEVOS:
...
const PagexDetalle = () => (
  <FilterProvider config={PagexDetalleConfig}>
    <ModalProvider modalsConfig={PagexDetalleConfig.modalsConfig}>
      <PagexDetallePageContent />
      <ModalManager />
    </ModalProvider>
  </FilterProvider>
);

export default PagexDetalle;
