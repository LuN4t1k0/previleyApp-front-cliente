...
const Input = forwardRef(({ className, inputClassName, hasError, enableStepper = true, type, ...props }, ref) => {
      ...
      {isPassword && (
        <div className="absolute bottom-0 right-0 flex h-full items-center justify-center px-3">
          <button
            ...
            className="h-fit w-fit rounded-sm outline-none transition-all text-gray-400 dark:text-gray-600 hover:text-gray-500 hover:dark:text-gray-500"
            type="button"
          ...
          >
            ...
            {typeState === "password" ? <RiEyeFill aria-hidden="true" className="size-5 shrink-0" /> : <RiEyeOffFill aria-hidden="true" className="size-5 shrink-0" />}
          </button>
        </div>
      )}
    </div>
